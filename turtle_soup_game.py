...
def analyze_story_structure(puzzle, truth):
    system_prompt = (
        ...
        "4. 关键地点：准确找出全部关键地点\n"
        "5. 时间线：按时间顺序列出全部关键事件\n"
        "6. 因果关系：列出全部关键事件之间的因果关系链\n"
        "7. 人物动机：准确找出全部人物之后并分析全部人物的心理动机\n"
        "8. 关键转折：指出故事中全部的关键转折点\n"
        "9. 情感发展：描述全部人物情感的变化过程\n"
        "请用JSON格式输出分析结果,包含以上全部维度。\n"
        "每个维度都要详细列出具体内容。"
    )
...
def check_if_game_should_end(puzzle, truth, user_input, story_analysis, history_text):
    ...
    return response.choices[0].message.content.strip()#.startswith("是")
